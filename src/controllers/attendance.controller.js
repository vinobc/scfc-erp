...
exports.getEnrolledStudents = async (req, res) => {
  try {
    ...
    const studentsResult = await db.query(
      `SELECT DISTINCT ON (sr.enrollment_number)
      ...
      [slot_year, semester_type, course_code, venue, slot_name, employee_id]
    );

    // Step 2: Get attendance status for these students on the target date
      ...
      const attendanceResult = await db.query(
        `SELECT student_id, status FROM attendance
         WHERE student_id = ANY($1)
           AND slot_year = $2 AND semester_type = $3
           AND course_code = $4 AND employee_id = $5
           AND venue = $6 AND slot_day = $7
           AND slot_name = $8 AND slot_time = $9
           AND attendance_date = $10`,
        [studentIds, slot_year, semester_type, course_code, employee_id, venue, slot_day, slot_name, slot_time, targetDate]
      );
      attendanceResult.rows.forEach(r => { attendanceMap[r.student_id] = r.status; });
    }

    // Step 3: Merge
    const result = { rows: studentsResult.rows.map(s => ({
      ...s,
      current_status: attendanceMap[s.student_id] || null,
      attendance_date: attendanceMap[s.student_id] ? targetDate : null
    })) };
...
exports.markAttendance = async (req, res) => {
  try {
    ...
    for (const record of attendance_records) {
      ...
      // Skip if this student already has an OD record (auto-generated by OD system)
      const existingOD = await db.query(
        `SELECT id FROM attendance
         WHERE student_id = $1 AND slot_year = $2 AND semester_type = $3
           AND course_code = $4 AND employee_id = $5 AND venue = $6
           AND slot_day = $7 AND slot_name = $8 AND slot_time = $9
           AND attendance_date = $10 AND od_activity_id IS NOT NULL`,
        [student_id, slot_year, semester_type, course_code, employee_id, venue,
         slot_day, slot_name, slot_time, attendance_date]
      );
      if (existingOD.rows.length > 0) {
        continue; // OD records cannot be overwritten by manual attendance
      }
...
exports.clearAttendance = async (req, res) => {
  try {
    ...
    // Delete all attendance records for this specific date and slot (except OD records)
    const result = await db.query(
