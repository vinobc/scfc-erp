  ...
  try {
    ...
    if (studentIds.length > 0) {
      const attendanceResult = await db.query(
        `SELECT student_id, status, is_od FROM attendance
      ...
      attendanceResult.rows.forEach(r => {
        attendanceMap[r.student_id] = { status: r.status, is_od: r.is_od };
      });
    }
    ...
    const result = { rows: studentsResult.rows.map(s => ({
      ...s,
      current_status: attendanceMap[s.student_id]?.status || null,
      is_od: attendanceMap[s.student_id]?.is_od || false,
...
exports.markAttendance = async (req, res) => {
  try {
    ...
    for (const record of attendance_records) {
      ...
      // Insert or update attendance record (preserves is_od and od_activity_id if set)
      await db.query(
        ...
        for (const slot of individualSlots) {
          ...
          const attResult = await db.query(`
            SELECT
              ...
              END as attendance_percentage,
              COUNT(a.id) as total_classes,
              COUNT(CASE WHEN a.status = 'present' OR a.is_od = true THEN 1 END) as present_classes
            FROM attendance a
            WHERE a.student_id = $1
              AND a.course_code = $2
              AND a.slot_year = $3
...
exports.getStudentAttendanceReport = async (req, res) => {
  try {
    ...
    const presentClasses = attendanceResult.rows.filter(r => r.status === 'present' || r.is_od === true).length;
