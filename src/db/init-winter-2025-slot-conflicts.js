...
// Initialize Winter 2025-26 slot conflicts based on clash table
async function initWinter2025SlotConflicts() {
  try {
    console.log("Initializing Winter 2025-26 slot conflicts...");

    // Winter 2025-26 conflicts (same as Fall, plus new D1+TA1 and D2+TA2 combinations)
    const winter2025Conflicts = [
      ...
      // NEW: D1+TA1 conflicts = D1 conflicts + TA1 conflicts (3-credit combined slot)
      { slot: "D1+TA1", conflicting_slot: "L3+L4" },
      { slot: "D1+TA1", conflicting_slot: "L13+L14" },
      { slot: "D1+TA1", conflicting_slot: "L7+L8" },
      ...
      // NEW: D2+TA2 conflicts = D2 conflicts + TA2 conflicts (3-credit combined slot)
      { slot: "D2+TA2", conflicting_slot: "L23+L24" },
      { slot: "D2+TA2", conflicting_slot: "L33+L34" },
      { slot: "D2+TA2", conflicting_slot: "L27+L28" },
    ];

    // Insert Winter 2025-26 conflicts (bidirectional)
    for (const conflict of winter2025Conflicts) {
      ...
      await db.query(
        `INSERT INTO slot_conflict
         ...
         DO NOTHING`,
        ["2025-26", "WINTER", conflict.slot, conflict.conflicting_slot]
      );
      ...
      await db.query(
        `INSERT INTO slot_conflict
         ...
         ON CONFLICT (slot_year, semester_type, slot_name, conflicting_slot_name)
         DO NOTHING`,
        ["2025-26", "WINTER", conflict.conflicting_slot, conflict.slot]
      );
    }

    console.log("Winter 2025-26 slot conflicts initialized successfully.");
    console.log(`Total conflicts inserted: ${winter2025Conflicts.length * 2} (bidirectional)`);
  } catch (error) {
    console.error("Error initializing Winter 2025-26 slot conflicts:", error);
  }
}

// Run the initialization
initWinter2025SlotConflicts()
  .then(() => process.exit(0))
  .catch((error) => {
